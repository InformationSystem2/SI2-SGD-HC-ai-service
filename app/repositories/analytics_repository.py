...
def get_plan_usage(db: Session, tenant_id: str):
    query = text("""
        WITH plan_limits AS (
            SELECT pl.resource_key, pl.resource_value
            FROM plan_limits pl
        ...
        counts AS (
            SELECT
                (SELECT COUNT(*) FROM users WHERE tenant_id = :tenant_id) AS user_count,
                ...
                (SELECT COALESCE(SUM(file_size_bytes), 0) FROM documents WHERE tenant_id = :tenant_id)
                    + (SELECT COALESCE(SUM(file_size_bytes), 0) FROM dicom_instances WHERE tenant_id = :tenant_id)
                    AS storage_bytes,
                (SELECT COALESCE(call_count, 0) FROM api_call_usage
                    WHERE tenant_id = :tenant_id AND year_month = to_char(CURRENT_DATE, 'YYYY-MM'))
                    AS api_calls,
                (SELECT COALESCE(SUM(ocr.pages_processed), 0) FROM document_ocr_metadata ocr
        ...
        SELECT
            c.user_count, c.patient_count, c.document_count,
            ...
            MAX(CASE WHEN pl.resource_key = 'maxDocumentTemplates' THEN pl.resource_value END) AS max_templates,
            ...
            MAX(CASE WHEN pl.resource_key = 'maxApiCallsPerMonth' THEN pl.resource_value END) AS max_api_calls,
            MAX(CASE WHEN pl.resource_key = 'maxOcrPagesPerMonth' THEN pl.resource_value END) AS max_ocr_pages
        FROM counts c, plan_limits pl
